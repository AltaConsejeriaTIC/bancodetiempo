@extends('emails.partial.layout')
    @section('title')
        ...
        <br>
        <h3>{{$userauth->first_name}} está @if($service->user->gender == 'male') interesado @elseif($service->user->gender == 'female') interesada @else interesadx @endif en tu oferta {{$service->name}}</h3>
    @endsection
    @section('content')
        
        <div style='width: 10%;float: left;margin-right: 5%;'>
            <img src="{{url('/')}}/{{$userauth->avatar}}" alt="" style="width:100%">
        </div>
